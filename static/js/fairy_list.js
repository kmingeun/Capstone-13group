...
function transformPrev(event) {
   ...
   * liList.length * 380 에서 380 은 각 li 요소의 실질 너비(margin 포함)
   * activeLi 는 data-position 에 있는 현재 위치 
   * 즉, liList.length * 380 + Number(activeLi) 는 현재 위치부터 오른쪽으로 나열되야 하는 나머지 카드들의 너비
   */

  /* classList.clientWidth < (liList.length * 380 + Number(activeLi)) 의미는
   * 오른쪽으로 나열될 카드들이 넘친 상태이므로, 왼쪽으로 이동이 가능함
   */
  
   if (classList.clientWidth < (liList.length * 380 + Number(activeLi))) {
       // 위치를 왼쪽으로 380 이동 (-380px)
       activeLi = Number(activeLi) - 380;

       /* 위치를 왼쪽으로 380 이동 (-380px)
       * 해당 위치는 변경된 activeLi 값이 적용된 liList.length * 380 + Number(activeLi) 값임
...
for (let i = 0; i < slidePrevList.length; i++) {
    ...
    if (classList.clientWidth < (liList.length * 380)) {
